...
const CustomersPage = () => {
  ...
  const [totalItems, setTotalItems] = useState<number>(10); // Track total items for skeleton rows
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try {
      ...
      const customersResponse = await fetch(
        `/api/customer?page=${currentPage}&limit=${itemsPerPage}`, // Corrected endpoint
        {
          method: "GET",
          headers: {
      ...
      console.log("API Response:", customersResult); // Debug log
      ...
      setTotalItems(customersResult.data.pagination?.total || itemsPerPage);
    } catch (error) {
  ...
  const renderSkeletonRows = () => {
    ...
    return Array.from({ length: rowCount }).map((_, index) => (
      <TableRow key={`skeleton-row-${index}`} className="border-white/10">
        ...
        {showActionsColumn && (
          <TableCell>
            <Skeleton className="h-8 w-8 bg-white/10" /> {/* Fixed typo */}
          </TableCell>
        )}
      </TableRow>
    ));
