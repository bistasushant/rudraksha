...
export default function ShopPage() {
  ...
  // Modified price filter states for dual-thumb slider
  ...
  // Price filter handlers with debounce
  ...
  const handlePriceInputChange = (type: "min" | "max", value: string) => {
    ...
    if (type === "min") {
      const validMin = Math.max(0, Math.min(numValue, priceRange[1] - 10));
      setMinPrice(validMin.toString());
      updatePriceRangeWithDebounce(validMin, priceRange[1]);
    } else {
      const validMax = Math.min(5000, Math.max(numValue, priceRange[0] + 10));
  ...
  // Comment out search functionality for future use
  /* 
  ...
  */
  ...
  if (errorProducts || errorCategories) {
    return (
      <div className="container mx-auto px-4 py-10 text-red-500">
  ...
  return (
    <div className="container mx-auto px-4 py-8 sm:px-6 overflow-x-hidden">
            ...
            {/* 
              <div className="relative w-full">
                ...
                <Input
                  ...
                  className="pl-10 border-2 text-base sm:text-lg placeholder:text-sm sm:placeholder:text-md w-full h-[40px]"
                  value={searchQuery}
