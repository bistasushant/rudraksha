...
const SideBar = ({
  ...
  // Local state to display the user's email
  ...
  // Sync email and image when user changes
  useEffect(() => {
    if (!isLoading && admin) {
    ...
    >
      ...
      <div className="mt-auto border-t border-white/10 p-4">
        <div className="flex items-center gap-3">
          <Avatar>
            <AvatarImage src={displayImage} alt={admin?.email || "Profile"} />
            <AvatarFallback className="bg-purple-600">
              {isLoading ? "..." : admin?.email?.[0] || "U"}
            </AvatarFallback>
          </Avatar>
